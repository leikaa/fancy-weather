...
export default function renderForecastInfo(currently, daily, city, timezone, language, country, countryMap) {
  const {
    summary, icon, apparentTemperature, humidity, windSpeed, temperature,
  } = currently;
  ...
  const time = new Date().toLocaleString(language, timeOptions);
  renderTime(time, language, timezone, date);

  // location name
  const countryName = countryMap[country.toUpperCase()] ? countryMap[country.toUpperCase()] : countryMap.default;
